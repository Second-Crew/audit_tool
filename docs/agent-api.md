...
Rendered evidence is optional and source-labeled. A page with insufficient text in fetched HTML may be rendered using the configured browser service; if it remains incomplete, content-based grades are withheld. `coverage.renderedPages` and `coverage.contentCoverage` report the observed sample. Rendered DOM findings carry a validation limitation and cannot become outreach-eligible on this methodology version. See [rendered evidence](rendered-evidence.md).
