...
export function validateAgentInput(body) {
  ...
  if (Object.keys(body).some(key => !['url','companyName','profile','maxPages'].includes(key))) throw new AgentError('unsupported_field',400);
  ...
  return { url, companyName: body.companyName || '', profile: body.profile || 'auto', maxPages: body.maxPages ?? 50 };
}
