...
function inferPageType(url, title, headings, text) {
  ...
  for (const field of [new URL(url).pathname, (headings.h1 || []).join(' '), title]) {
...
function summarizeContentEvidence(pages) {
  ...
  return {pages:pages.length,usablePages:pages.length-sparse.length,sparsePages:sparse.length,renderedPages:rendered.length,coverage,
    status:coverage>=0.8?'sufficient':'incomplete',
    limitation:coverage>=0.8?null:'Some sampled pages returned too little extractable content; JavaScript rendering may be required.'};
}
