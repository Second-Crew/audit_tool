...
export function buildFreshnessPattern(year = new Date().getFullYear()) {
  return new RegExp(`\\b(?:last\\s+updated|updated\\s+(?:on|in)|published\\s+(?:on|in))\\s*:?\\s*(?:(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\s+\\d{1,2},?\\s*|\\d{1,2}[/-]\\d{1,2}[/-])?(?:${year - 1}|${year})\\b`, 'i');
}
...
export function extractSiteSignals(crawl, input = {}) {
  ...
  return {
    ...
    commerce: summarizeCommerce(pages, input),
...
function extractPageSignals(page, input) {
  ...
  const directAnswerCount = countDirectAnswers($);
  const hasVisibleFaq = detectFaq($, page.url, title, headings, directAnswerCount);
  ...
  // Header/footer contacts are valid public identity evidence, even though
  // their boilerplate must stay out of content-purpose and answer checks.
  // cleanContent has already removed scripts and explicitly hidden elements.
  const contactBody = $('body').clone();
  contactBody.find('p,div,li,a,br').prepend(' ').append(' ');
  const contactText = normalizeText(contactBody.text());
  const phones = Array.from(new Set(contactText.match(PHONE_PATTERN) || []));
  const emails = Array.from(new Set(contactText.match(EMAIL_PATTERN) || []));
...
function summarizeContent(pages, allText, input) {
  // A bare FAQ URL or a repeated FAQ label in a shared template is not
  // evidence that this page actually contains usable questions and answers.
  const faqPages = pages.filter((page) => page.text.length >= 80 && page.hasVisibleFaq);
  ...
  const comparisonPages = pages.filter((page) => /(?:compare|comparison|versus|\bvs\b|alternatives?)/i.test(`${new URL(page.url).pathname} ${page.title} ${(page.headings.h1 || []).join(' ')}`));
  const educationalPages = pages.filter((page) => page.contentType === 'education');
  const hasDirectAnswers = pages.some((page) => page.directAnswerCount > 0);
  ...
  const hasFreshnessSignals = pages.some((page) => buildFreshnessPattern().test(page.text));
...
function summarizeEntity(pages, allText, input) {
  ...
  const trustPages = pages.filter((page) => /\b(?:case-stud(?:y|ies)|testimonials?|reviews?|customer-stor(?:y|ies)|portfolio|press|awards?|certifications?)\b/i.test(`${new URL(page.url).pathname} ${page.title}`));
...
function summarizeCommerce(pages, input) {
  const productPages = pages.filter((page) => page.contentType === 'product');
  const productSchemaNodes = productPages.flatMap((page) => page.schema.nodes);
  const policyHeadings = pages.map((page) => `${new URL(page.url).pathname} ${page.title} ${(page.headings.h1 || []).join(' ')}`);
  const hasShipping = policyHeadings.some((heading) => /\bshipping\b|\bdelivery\b/i.test(heading)) || pages.some((page) => page.contentType === 'product' && /\bshipping\b|\bdelivery\b/i.test(page.text));
  const hasReturns = policyHeadings.some((heading) => /\breturns?\b|\brefunds?\b/i.test(heading)) || pages.some((page) => page.contentType === 'product' && /\breturns?\b|\brefunds?\b/i.test(page.text));
  return {
    likelyEcommerce: resolveSiteType(pages, input).ecommerce.applicable,
    hasProductSchema: productSchemaNodes.some((node) => normalizeTypeList(node.type).some((type) => /^(?:Product|ProductGroup)$/.test(type))),
    hasOfferSchema: productSchemaNodes.some((node) => normalizeTypeList(node.type).some((type) => /^(?:Offer|AggregateOffer)$/.test(type))),
    hasReviews: pages.some((page) => page.contentType === 'product' && /\b(?:\d+(?:\.\d+)?\s*(?:out of 5|\/\s*5|stars?)|rated\s+\d+(?:\.\d+)?|\d+\s+(?:verified\s+)?(?:customer\s+)?reviews?)\b/i.test(page.text)),
    hasShippingReturns: hasShipping && hasReturns,
...
function summarizeSeo(pages, titles, descriptions) {
  ...
  const importantNoindexPages = pages.filter((page) => !page.indexable && (isCriticalPageUrl(page.url) || ['service', 'product', 'location'].includes(page.contentType)));
  ...
  return {
    ...
    h1Coverage: pages.length ? pagesWithH1.length / pages.length : 0,
  };
}
...
function detectFaq($, url, title, headings, directAnswerCount) {
  if (!directAnswerCount) return false;
  const intent = `${new URL(url).pathname} ${title} ${(headings.h1 || []).join(' ')}`;
  if (/(?:^|[\s/])faqs?(?:$|[\s/])|frequently asked|questions and answers|\bq&a\b/i.test(intent)) return true;
  const copy = $.root().clone();
  copy.find('nav,header,footer,aside,[role="navigation"],[role="banner"],[role="contentinfo"],[hidden],[aria-hidden="true"]').remove();
  const main = copy.find('main,[role="main"]').first();
  const root = main.length ? main : copy.find('body');
  return root.find('[class*="faq" i], [id*="faq" i]').length > 0
    || root.find('h2,h3,h4').filter((_, element) => /frequently asked|\bfaq\b|questions and answers|\bq&a\b/i.test(normalizeText($(element).text()))).length > 0;
}

function countDirectAnswers($) {
  const copy = $.root().clone();
  copy.find('nav,header,footer,aside,[role="navigation"],[role="banner"],[role="contentinfo"]').remove();
  const main = copy.find('main,[role="main"]').first();
  const root = main.length ? main : copy.find('body');
  let count = 0;
  root.find('h2,h3,h4,dt').each((_, element) => {
    const heading = $(element);
    if (!/\?$/.test(normalizeText(heading.text()))) return;
    const answer = normalizeText(heading.next().text());
    if (answer.length >= 40 && answer.split(/\s+/).length >= 7) count += 1;
  });
  return count;
}
...
function inferPageType(url, title, headings, text) {
  ...
  if (segments.some((segment) => /^(?:privacy|privacy-policy|terms|terms-of-service|terms-and-conditions|legal|cookie-policy)$/.test(segment))) return 'legal';
  if (segments.some((segment) => /^(?:products?|shop)$/.test(segment))) return 'product';
  const rules = [
    ['service', /\bservices?\b|\bsolutions?\b|what we do/i],
    ['contact', /\bcontact\b|get-in-touch|book an appointment/i],
    ['about', /(?:^|\/)(?:about|about-us)(?:\/|$)|\babout (?:us|our company)\b|\bour (?:story|team)\b/i],
  ...
  if (segments.length > 0 && /\badd to (?:cart|bag)\b/i.test(text) && /\bsku\b/i.test(text)) return 'product';
