...
import { summarizeAuditForResponse } from './summarize.js';
...
async function insertAudit(config, clientId, audit, compatibility) {
  const payload = {
    ...
    report: {
      ...
      // Everything the workspace UI needs to reopen this audit with full
      // tabs, action plan, and exports — no re-crawl required.
      workspace: summarizeAuditForResponse(audit),
      ai_bot_access: compatibility.aiReadiness?.features?.aiBotAccess || null,
    },
