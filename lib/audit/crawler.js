...
// Must match the token in the fetcher's User-Agent header so sites can
// target the crawler with a robots.txt group.
const CRAWLER_UA_TOKEN = 'SecondCrewAuditBot';
...
export async function crawlSite(inputUrl, limitOverrides = {}, onProgress = null) {
  ...
  // Per-host politeness: space page requests out even across concurrent
  // workers (the whole crawl targets a single host).
  let nextRequestAt = 0;
  async function politeWait() {
    const now = Date.now();
    const waitMs = Math.max(0, nextRequestAt - now);
    nextRequestAt = Math.max(nextRequestAt, now) + limits.requestDelayMs;
  ...
  async function worker() {
    while (Date.now() < deadline && pages.length < limits.maxPages) {
      // Reserve capacity for requests already in flight. A successful response
      // from each of those workers can otherwise exceed maxPages.
      if (pages.length + inFlight >= limits.maxPages) {
        await sleep(50);
        continue;
      }
      ...
      if (!isUrlAllowedByRobots(nextUrl, auxiliary.robots.parsed, origin)) {
        blockedByRobots.push(nextUrl);
        continue;
      }
      ...
      try {
        ...
        if (!/html|xhtml/i.test(contentType)) {
          errors.push({url:nextUrl,status:result.status,error:'Unsupported content type'});
          continue;
        }
        if (!isSameSite(result.url, startUrl)) {
          errors.push({url:nextUrl,status:result.status,error:'Redirect left audited site'});
          continue;
        }
...
function collectSeedUrls(startUrl, auxiliary) {
  ...
  return Array.from(seedUrls).sort((a, b) => {
    if (a === canonicalizeUrl(startUrl)) return -1;
    if (b === canonicalizeUrl(startUrl)) return 1;
    return scoreUrlPriority(b) - scoreUrlPriority(a);
  });
}
...
function scoreUrlPriority(url) {
  ...
  let score = pathname === '/' ? 1000 : 0;

  PRIORITY_PATH_PATTERNS.forEach((pattern, index) => {
    if (pattern.test(pathname)) score = Math.max(score, 100 - index);
  });
...
function isUrlAllowedByRobots(url, parsedRobots, origin) {
  try {
    const parsed = new URL(url);
    return isAllowedByRobots(parsedRobots, CRAWLER_UA_TOKEN, parsed.pathname + parsed.search, origin);
  } catch {
    return true;
  }
}
