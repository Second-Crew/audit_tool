export function buildCompatibilityResponse(audit) {
  ...
  const aeoGeoAnalysis = {
    ...
    llmContext: {
      ...
      prediction: scores.aeoGeo == null ? 'Insufficient extracted content to assess on-site AI answer readiness.' : scores.aeoGeo >= 80
...
function buildExecutiveSummary(audit) {
  ...
  if (scores.overall == null) return `${audit.input.companyName || audit.primary.signals.domain} could not be graded because too many sampled pages returned insufficient extractable content. Render those pages and repeat the audit before recommending content changes.`;
...
function buildIndustryInsight(audit) {
  ...
  if (audit.primary.signals.contentEvidence?.status === 'incomplete') return `For ${target}, only ${audit.primary.signals.contentEvidence.usablePages} of ${audit.primary.signals.contentEvidence.pages} sampled pages had enough extractable text for a reliable on-site assessment. Review incomplete pages and repeat the audit before business recommendations.`;
