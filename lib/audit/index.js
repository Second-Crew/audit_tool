import { renderSparsePages } from './render.js';
...
export async function runAudit(input, onProgress = null) {
  ...
  emit({ stage: 'render' });
  await renderSparsePages(primaryCrawl);
  emit({ stage: 'scoring' });
  if (!primaryCrawl.pages.length) {
    throw new Error('No pages could be retrieved from the primary website. No website scores were calculated. Check crawl access and try again.');
  }
  ...
  emit({ stage: 'llm' });
  // Keep experimental judgments separate from production grades/narrative until
  // evaluated against labeled audits. Run both providers within the same stage.
  [audit.llmInsights, audit.semantic] = await Promise.all([
    input.skipNarrative || primarySignals.contentEvidence.status === 'incomplete' ? Promise.resolve({enabled:false,status:'skipped',reason:'Insufficient extracted content for a reliable narrative'}) : generateAuditNarrative(audit),
    evaluateTypeSafe(primaryCrawl),
  ]);
  audit.elapsedMs = Date.now() - startedAt;

  audit.assessment = buildAgentResult(audit, {profile:input.profile || 'auto'});
  if (input.skipNarrative) return {audit, compatibility:null};
  emit({ stage: 'report' });
  const compatibility = buildCompatibilityResponse(audit);
  compatibility.html = generateEvidenceReport(audit, compatibility);
  compatibility.markdown = buildMarkdownReport({
    companyName: audit.input.companyName || primarySignals.domain,
    domain: primarySignals.domain,
    startUrl: primarySignals.startUrl,
    createdAt: audit.createdAt,
    pageCount: primarySignals.pageCount,
    siteType: primarySignals.siteType,
    contentEvidence: primarySignals.contentEvidence,
    scores: primaryScoring.scores,
    findings: primaryScoring.findings,
    categoryDetails: primaryScoring.categoryDetails,
    competitorComparison,
    aiInsights: compatibility.aiInsights,
    actionPlan: buildActionPlan(
      { aiInsights: compatibility.aiInsights, scores: compatibility.scores },
      { pages: primarySignals.pages.map(summarizePageForPlan), contentEvidence: primarySignals.contentEvidence, crawl: primarySignals.crawl },
      primaryScoring.categoryDetails,
      primaryScoring.findings
    ),
  });
...
function summarizePageForPlan(page) {
  return {
    url: page.url,
    status: page.status,
    title: page.title,
    ...
    contentEvidenceIncomplete: page.text.length < 80,
...
function normalizeInput(input = {}) {
  ...
  return {
    ...
    ecommerceFunctionality: input.ecommerceFunctionality || 'auto',
