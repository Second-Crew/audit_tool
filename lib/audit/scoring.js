...
function scoreStructuredData(signals, findings) {
  ...
  points += addCheck(checks, meaningful && (signals.schema.hasService || signals.schema.hasProduct), 18, 'Service or Product schema exists', schemaEvidence(signals));
  points += addCheck(checks, meaningful && (signals.schema.hasFAQ || signals.schema.hasArticle), 14, 'FAQ or Article schema supports answer extraction', schemaEvidence(signals));
  ...
  points += addCheck(checks, meaningful && signals.schema.hasReview, 10, 'Review or rating schema exists', schemaEvidence(signals));
...
function toCategory(name, points, checks) {
  ...
  return {
    name,
    score: coverage >= 0.8 && max ? Math.round((points / max) * 100) : null,
    coverage,
