export function scoreSite(signals, pageSpeed = { scores: {}, metrics: {}, available: false }) {
  ...
  return {
    scores: {
      overall: [crawl,schema,content,entity,seo,experience,vertical].some(c => c.score == null) ? null : overall,
      aeoGeo: [crawl,schema,content,entity,vertical].some(c => c.score == null) ? null : aeoGeo,
      aiReadiness: [crawl,schema,content,entity,vertical].some(c => c.score == null) ? null : aiReadiness,
      seo: [seo,crawl,schema,experience].some(c => c.score == null) ? null : seoComposite,
...
function scoreCrawlability(signals, findings) {
  ...
  const chatGptAllowed = signals.robots.botAccess['ChatGPT-User']?.allowed ?? null;
  ...
  points += addCheck(checks, chatGptAllowed, 8, 'ChatGPT-User robots permission on sampled paths', signals.robots.botAccess['ChatGPT-User']?.evidence);
...
function scoreStructuredData(signals, findings) {
  ...
  points += addCheck(checks, meaningful, 18, 'JSON-LD schema exists', `${signals.schema.count} schema nodes found`);
  points += addCheck(checks, meaningful && signals.schema.hasLocalBusiness, 12, 'Organization or LocalBusiness schema exists', schemaEvidence(signals));
  if (signals.content.servicePages.length || signals.siteType?.ecommerce?.applicable) {
    points += addCheck(checks, meaningful && (signals.schema.hasService || signals.schema.hasProduct), 18, 'Relevant Service or Product schema exists', schemaEvidence(signals));
  }
  if (signals.content.educationalPages?.length || signals.content.faqPages.length) {
    points += addCheck(checks, meaningful && (signals.schema.hasFAQ || signals.schema.hasArticle), 14, 'Article or FAQ schema type found for eligible content', schemaEvidence(signals));
  }
  points += addCheck(checks, meaningful && signals.schema.hasBreadcrumb, 8, 'Breadcrumb schema clarifies site hierarchy', schemaEvidence(signals));
  ...
  if (!signals.schema.found) {
    findings.push(makeFinding({
      ...
      description: 'No JSON-LD was observed on crawled pages. This is a machine-readable data gap, not evidence of poor search ranking or AI visibility.',
      recommendation: 'Start with accurate Organization or WebSite facts. Add Service, Article, Product, or Breadcrumb markup only where the visible page content supports it, then validate eligible features with Google’s Rich Results Test.',
...
function scoreEntityTrust(signals, findings) {
  ...
  points += addCheck(checks, Boolean(contactEvidence), 14, 'Public contact path exists', contactEvidence || 'No contact path observed on sampled pages');
  ...
  points += addCheck(checks, Boolean(contactEvidence), 16, 'Contact method is accessible', contactEvidence || 'No contact method observed on sampled pages');
...
function scoreEcommerceReadiness(signals, findings) {
  ...
  points += addCheck(checks, signals.commerce.productPageCount > 0, 20, 'Product/category pages were found', `${signals.commerce.productPageCount} product-like pages`);
  ...
  if (!signals.commerce.hasProductSchema) {
    findings.push(makeFinding({
      ...
      recommendation: 'Add accurate Product and Offer markup to relevant product pages. Include AggregateRating only when eligible product reviews are visible and meet the applicable guidelines.',
...
function scoreGeneralReadiness(signals, findings) {
  ...
  points += addCheck(checks, Boolean(contactEvidence), 20, 'Public contact path exists', contactEvidence || 'No contact path observed on sampled pages');
