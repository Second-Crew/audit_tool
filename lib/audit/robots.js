...
export function getBotAccess(robotsTxt, origin, urls = [origin + '/'], status = 200) {
  ...
  return bots.reduce((access, bot) => {
    const known = (status >= 200 && status < 300) || status === 404 || status === 410;
    const pages = urls.map(url => {
      const parsedUrl = new URL(url);
      const path = parsedUrl.pathname + parsedUrl.search;
      return { url, path, allowed: known ? isAllowedByRobots(parsed, bot, path, origin) : null };
    });
    const allowed = known ? pages.every(page => page.allowed) : null;
    access[bot] = {
      allowed, status: known ? (allowed ? 'allowed' : 'blocked') : 'unknown', pages,
      evidence: known ? `${bot}: ${pages.filter(p => p.allowed).length}/${pages.length} sampled paths permitted by robots.txt; retrieval not verified` : `${bot}: robots.txt could not be assessed`,
    };
    return access;
  }, {});
}
