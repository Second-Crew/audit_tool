...
<body>
  <main class="wrap">
    ...
    ${primary.signals.contentEvidence?.status === 'incomplete' ? `<section class="card"><strong>Content assessment unavailable</strong><p>Only ${primary.signals.contentEvidence.usablePages} of ${primary.signals.contentEvidence.pages} sampled pages had enough extractable text. Review incomplete pages and repeat the audit before using content recommendations or an overall grade.</p></section>` : ''}
    ${primary.signals.contentEvidence?.status === 'sufficient' && scores.overall == null ? `<section class="card"><strong>Overall assessment unavailable</strong><p>Page content was sufficient, but ${escapeHtml(categoryRows.filter(category => category.score == null).map(category => category.name).join(', ') || 'a required category')} could not be fully measured. Retry unknown checks before using a composite grade or ranked implementation plan.</p></section>` : ''}
    <section class="grid">
    ...
    <section class="section">
      <h2>${scores.overall == null ? 'Assessment Recovery' : 'Recommended Roadmap'}</h2>
      <div class="cols">
        ${scores.overall == null ? '<div class="card">Complete unavailable evidence or measurements and rerun the audit before ranking website changes.</div>' : (compatibility.aiInsights.roadmap || []).map((item) => `
          <div class="card">
