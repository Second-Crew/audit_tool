...
async function getPageSpeedData(url, strategy) {
  let result = await requestPageSpeed(url, strategy);
  if (result?.unavailable && (result.status === 0 || result.status === 429 || result.status >= 500)) {
    await new Promise((resolve) => setTimeout(resolve, 1500));
    result = await requestPageSpeed(url, strategy);
  }
  return result;
}
