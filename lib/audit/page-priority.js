...
export function isCriticalPageUrl(url) {
  try {
    ...
    return !/^(?:cart|checkout|basket|account|login|sign-in|register|search|faq|faqs|blog|blogs|news|articles|guides|resources|tags?|categories|privacy|terms|legal|feed|sitemap\.xml|robots\.txt|llms\.txt)$/.test(segments[0]);
  } catch {
    return false;
  }
}
