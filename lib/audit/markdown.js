...
export function buildMarkdownReport(data) {
  ...
  if (contentEvidence?.status === 'incomplete') lines.push(`- **Content assessment:** Unavailable; ${contentEvidence.usablePages}/${contentEvidence.pages} sampled pages had extractable text. Render and reassess before content recommendations.`);
  lines.push('');
