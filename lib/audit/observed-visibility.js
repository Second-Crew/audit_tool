...
export function scoreObservedVisibility(observations, targetDomain, {
  ...
  const rows = (observations || []).filter((row) => row.valid !== false && row.engine === engine && row.queryId && row.runId && typeof row.answerShown === 'boolean');
  const queries = new Map();
  for (const row of rows) {
    if (!queries.has(row.queryId)) queries.set(row.queryId, new Map());
    queries.get(row.queryId).set(row.runId, row);
  }
  const completeQueries = [...queries.entries()].filter(([, runs]) => runs.size >= minRunsPerQuery);
  if (completeQueries.length < minQueries) {
    return {
      status: 'not_assessed',
      score: null,
      engine,
      queryCount: completeQueries.length,
      requiredQueries: minQueries,
