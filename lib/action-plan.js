export function buildActionPlan(report, primary, categoryDetails, findings) {
  if (primary?.contentEvidence?.status === 'incomplete') {
    ...
    const generalTasks = [{
      ...
      evidence: `${usablePages} of ${pages} sampled pages had enough content${rendering ? `; ${rendering.succeeded} rendered successfully, ${rendering.failed} failed, ${rendering.skipped} skipped` : ''}`,
      impact: 'High',
...
function buildCategoryTasks(categoryDetails) {
  return Object.values(categoryDetails || {})
    .flatMap((category) => (category.checks || [])
      .filter((check) => ['failed', 'partial'].includes(check.status))
      .map((check, index) => ({
