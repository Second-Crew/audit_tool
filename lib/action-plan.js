export function buildActionPlan(report, primary, categoryDetails, findings) {
  ...
  if (report?.scores?.overall === null) {
    ...
    const measurementTask = {
      id: 'measurement-recovery',
      title: 'Verify unavailable measurements',
      detail: 'Retry or independently verify unknown checks before drawing conclusions from them.',
      evidence: checks.length ? checks.join('; ') : 'Composite ranking and AI visibility grades are withheld pending outcome calibration.',
      impact: 'Low',
      effort: 'Review',
      source: 'Measurement coverage',
    };
    const generalTasks = [...buildGeneralTasks(findings), measurementTask];
    const allPagePlans = buildPagePlans(primary?.pages || []);
    const pagePlans = allPagePlans.slice(0, 8);
    const allTasks = [...generalTasks, ...pagePlans.flatMap((page) => page.tasks)];
    return {
...
function categoryTaskTitle(label) {
  ...
  if (/H1 heading/i.test(label)) return 'Review heading structure on core templates';
...
function categoryTaskImpact(label) {
  if (/H1 heading|contact page|contact identity|breadcrumb/i.test(label)) return 'Low';
...
function recommendActionForCheck(label, categoryName) {
  ...
  if (text.includes('robots') || text.includes('googlebot') || text.includes('oai-searchbot')) return 'Review robots.txt permissions for Googlebot and OAI-SearchBot on public pages intended for search discovery; permission alone does not prove indexing or citation.';
